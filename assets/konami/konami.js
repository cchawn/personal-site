$(document).ready(() => {
  const $ken = $('.ken');
  let $fireballPos;

  const COUNT = {};
  const HIRE_ME_PLS = 'hiremepls';
  let E_COUNT = 0;
  for (let i = 0; i < HIRE_ME_PLS.length; i += 1) {
    COUNT[HIRE_ME_PLS[i]] = true;
  }

  function hadoken() {
    $ken.show();
    $ken.addClass('hadoken');
    setTimeout(() => {
      $ken.removeClass('hadoken');
    }, 500);
    setTimeout(() => {
      const $fireball = $('<div/>', { class: 'fireball' });
      $fireball.appendTo($ken);

      const isFireballCollision = () => $fireballPos.left + 75 > $(window).width();

      const explodeIfCollision = setInterval(() => {
        $fireballPos = $fireball.offset();

        if (isFireballCollision()) {
          $fireball.addClass('explode').removeClass('moving').css('marginLeft', '+=22px');
          clearInterval(explodeIfCollision);
          setTimeout(() => {
            $fireball.remove();
          }, 500);
        }
      }, 50);

      setTimeout(() => {
        $fireball.addClass('moving');
      }, 20);

      setTimeout(() => {
        $fireball.remove();
        clearInterval(explodeIfCollision);
      }, 3020);
    }, (250));
  }

  function reassembleMessage(char, str, element) {
    element.css({ position: 'absolute' });
    if (str.indexOf(char) !== '-1') {
      if (COUNT[char] || (char === 'e' && E_COUNT === 1)) {
        switch (char) {
          case 'h':
            element.animate({ top: '0px', left: '0', opacity: 1 }, 1000);
            break;
          case 'i':
            element.animate({ top: '0px', left: '12px', opacity: 1 }, 1000);
            break;
          case 'r':
            element.animate({ top: '0px', left: '17px', opacity: 1 }, 1000);
            break;
          case 'e':
            if (E_COUNT === 0) {
              E_COUNT = 1;
              element.animate({ top: '0px', left: '25px', opacity: 1 }, 1000);
            } else if (E_COUNT === 1) {
              E_COUNT = 2;
              element.animate({ top: '0px', left: '70px', opacity: 1 }, 1000);
            }
            break;
          case 'm':
            element.animate({ top: '0px', left: '50px', opacity: 1 }, 1000);
            break;
          case 'p':
            element.animate({ top: '0px', left: '90px', opacity: 1 }, 1000);
            break;
          case 'l':
            element.animate({ top: '0px', left: '105px', opacity: 1 }, 1000);
            break;
          case 's':
            element.animate({ top: '0px', left: '111px', opacity: 1 }, 1000);
            break;
          default:
          // do nothing;
        }
        COUNT[char] = false;
        return true;
      }
    }
    element.css({ top: '0', left: '0' });
    return false;
  }

  function explodeMessage() {
    $('.logo').addClass('spin');
    const $message = $('.message');
    let x;
    let y;
    const max = 100;
    $message.html($message.text().replace(/./g, '<span>$&</span>'));
    $message.children().each(function () {
      x = (Math.random() * max) - (max / 2);
      y = (Math.random() * max) - (max / 2);
      $(this).animate({
        top: `${y}vh`,
        left: `${x}vh`,
        opacity: 0,
      }, 2000, () => {
        reassembleMessage($(this).text(), HIRE_ME_PLS, $(this));
        $message.css({ left: '40%', width: '100px' });
      });
    });
  }

  new Konami((() => { // eslint-disable-line no-new
    hadoken();
    if (!$('.logo').hasClass('spin')) explodeMessage();
  }));
  console.log('⬆️ ⬆️ ⬇️ ⬇️');
});
